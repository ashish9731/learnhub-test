...
-- 5. Drop and recreate get_user_metrics function to resolve 42P13 error
DROP FUNCTION IF EXISTS public.get_user_metrics(uuid);

CREATE FUNCTION public.get_user_metrics(p_user_id uuid) RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
    -- Validate permissions
    IF auth.uid() != p_user_id AND
       (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = auth.uid()) != 'super_admin' AND
       NOT EXISTS (
           SELECT 1 FROM company_admins WHERE user_id = auth.uid() AND company_id = (
...
-- 6. Drop and recreate calculate_user_metrics function
...
-- 7. Create update_user_metrics_from_progress trigger function
CREATE FUNCTION public.update_user_metrics_from_progress() RETURNS trigger
    SECURITY DEFINER
AS $$
BEGIN
...
-- 8. Create trigger on podcast_progress for metrics update
...
-- 9. Create notify_progress_change function
CREATE FUNCTION public.notify_progress_change() RETURNS trigger
    SECURITY DEFINER
AS $$
BEGIN
...
-- 10. Create trigger for notifications
CREATE TRIGGER notify_progress_trigger
    AFTER INSERT OR UPDATE ON public.podcast_progress
    FOR EACH ROW
    EXECUTE FUNCTION public.notify_progress_change();

-- 11. Create create_or_update_user_metrics function
CREATE FUNCTION public.create_or_update_user_metrics(p_user_id uuid) RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
...
CREATE FUNCTION public.list_all_user_metrics() RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
    -- Validate super admin role
    IF (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = auth.uid()) != 'super_admin' THEN
...
CREATE FUNCTION public.list_company_user_metrics(p_company_id uuid) RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
...
CREATE FUNCTION public.get_admin_user_metrics() RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
...
CREATE FUNCTION public.get_user_podcast_progress(p_user_id uuid) RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
    -- Validate permissions
...
CREATE FUNCTION public.get_all_podcast_progress() RETURNS json
    SECURITY DEFINER
AS $$
BEGIN
    -- Validate super admin role
