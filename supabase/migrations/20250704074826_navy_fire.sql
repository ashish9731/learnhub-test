/*
  # Remove Hardcoded Email References and Fix Super Admin Access

  1. Security Changes
    - Remove hardcoded email references from RLS policies
    - Use role-based access control instead of email-based
    - Ensure super admin access is properly controlled

  2. Changes
    - Update all policies to use role-based checks
    - Remove any hardcoded email addresses
    - Fix super admin access control
*/

-- Drop all existing policies that might reference hardcoded emails
DO $$ 
DECLARE
    r RECORD;
BEGIN
...
-- Update any existing super admin users to have proper role
UPDATE users
SET role = 'super_admin'
WHERE email = 'admin@example.com';

-- Remove any test/example users
DELETE FROM users
