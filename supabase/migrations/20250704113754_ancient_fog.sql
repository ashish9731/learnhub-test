...
BEGIN
    ...
    FOR course_record IN (SELECT id FROM courses) LOOP
        ...
        FOR r IN (
            ...
            -- Update podcasts to use this category_id
            UPDATE podcasts 
    ...
    FOR course_record IN (
        ...
        FROM podcasts 
        WHERE category IS NULL OR category_id IS NULL
    ) LOOP
...
DELETE FROM podcasts WHERE title LIKE '%dummy%' OR title LIKE '%test%';
DELETE FROM courses WHERE title LIKE '%dummy%' OR title LIKE '%test%';
