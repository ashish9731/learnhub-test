...
-- Create policies for homepage_content (with checks to avoid duplicates)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'homepage_content' 
        AND policyname = 'homepage_content_select_all'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'homepage_content' 
        AND policyname = 'homepage_content_super_admin_access'
    ) THEN
...
-- Create policies for contact_messages (with checks to avoid duplicates)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'contact_messages' 
        AND policyname = 'contact_messages_super_admin_access'
    ) THEN
...
-- Create policies for faqs (with checks to avoid duplicates)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'faqs' 
        AND policyname = 'faqs_select_all'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'faqs' 
        AND policyname = 'faqs_super_admin_access'
    ) THEN
...
-- Create triggers for updated_at timestamps (with checks to avoid duplicates)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Create triggers for activity logging (with checks to avoid duplicates)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'log_homepage_content'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'log_contact_messages'
    ) THEN
...
-- Insert initial homepage content (only if table is empty)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM homepage_content LIMIT 1) THEN
...
-- Insert initial FAQs (only if table is empty)
DO $$
BEGIN
