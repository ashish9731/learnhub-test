/*
  # Fix RLS Policy Recursion and Performance Issues

  1. Problem
    - RLS policies are causing unnecessary re-evaluation for each row
    - Performance advisor shows warnings about `current_setting()` and `auth.<function>()` calls
    - Multiple permissive policies for the same role on some tables

  2. Solution
    - Drop all existing problematic policies
    - Create optimized policies that avoid unnecessary re-evaluation
...
-- Create optimized policies for companies table (consolidate multiple permissive policies)
...
-- Create optimized policies for content_categories table (consolidate multiple permissive policies)
