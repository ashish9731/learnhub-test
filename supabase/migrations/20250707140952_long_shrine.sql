/*
  # Fix Trigger Functions with OLD Reference Error

  1. Problem
    - Error: "42P01: missing FROM-clause entry for table 'old'"
    - Trigger functions incorrectly reference OLD as a table instead of a record
    - This causes authentication and user management operations to fail

  2. Solution
    - Fix all trigger functions that use OLD incorrectly
    - Ensure proper handling of OLD/NEW records in trigger functions
    - Improve error handling in all trigger functions
*/
...
-- Fix validate_user_company_assignment function to properly handle OLD/NEW records
...
-- Fix handle_new_user function to properly handle NEW record
...
-- Fix ensure_user_exists function to properly handle NEW record
...
-- Fix sync_podcast_category function to properly handle NEW record
