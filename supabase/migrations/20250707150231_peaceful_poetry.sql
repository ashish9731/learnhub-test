...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        -- For DELETE operations, use OLD record directly as a record variable, not as a table
        record_id := OLD.id;
        record_data := to_jsonb(OLD);
    ELSE
        -- For INSERT and UPDATE operations, use NEW record directly as a record variable, not as a table
        record_id := NEW.id;
