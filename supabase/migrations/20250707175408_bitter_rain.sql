/*
  # Fix User Authentication and Audit Logs

  1. Problem
    - Error: "null value in column "user_id" of relation "audit_logs" violates not-null constraint"
    - This is preventing user deletion and possibly other operations
    - Users are getting "Your account is not properly configured" error during login
    - Need to fix authentication flow and constraints

  2. Solution
    - Modify the audit_logs table to allow NULL values for user_id
    - Fix the handle_user_update function to handle NULL values properly
    - Update the validate_user_company_assignment function to be more lenient
    - Fix the assigned_users view to properly handle all user types
*/
...
DECLARE
  ...
  new_company TEXT;
BEGIN
  -- Convert values to text to avoid NULL issues
  old_email := COALESCE(OLD.email::text, 'NULL');
  new_email := COALESCE(NEW.email::text, 'NULL');
  old_role := COALESCE(OLD.role::text, 'NULL');
  new_role := COALESCE(NEW.role::text, 'NULL');
  old_company := COALESCE(OLD.company_id::text, 'NULL');
  new_company := COALESCE(NEW.company_id::text, 'NULL');

  -- Process changes and log them
  IF NEW.email IS DISTINCT FROM OLD.email THEN
    INSERT INTO audit_logs(user_id, action, old_value, new_value)
    VALUES (NEW.id, 'email_change', old_email, new_email);
  END IF;
  
  IF NEW.role IS DISTINCT FROM OLD.role THEN
    INSERT INTO audit_logs(user_id, action, old_value, new_value)
    VALUES (NEW.id, 'role_change', old_role, new_role);
  ...
  IF NEW.company_id IS DISTINCT FROM OLD.company_id THEN
    INSERT INTO audit_logs(user_id, action, old_value, new_value)
    VALUES (NEW.id, 'company_change', old_company, new_company);
  END IF;
...
-- Drop and recreate the user update trigger
