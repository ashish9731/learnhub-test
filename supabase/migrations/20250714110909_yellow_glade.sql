/*
  # Create podcast progress tracking table
  ...
  1. New Tables
    - `podcast_progress`
      ...
      - `current_time` (float, seconds played)
      - `duration` (float, total seconds)
      - `progress_percent` (integer, percentage complete)
...
CREATE INDEX IF NOT EXISTS idx_podcast_progress_user_podcast ON podcast_progress(user_id, podcast_id);

-- Add trigger for logging
CREATE TRIGGER log_podcast_progress
AFTER INSERT OR DELETE OR UPDATE ON podcast_progress
